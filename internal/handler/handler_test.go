...
func TestApplicationDetailsRendersEmptyServicesState(t *testing.T) {
	...
	if strings.Contains(body, `id="deployment-tab"`) || strings.Contains(body, `id="deployment-panel"`) {
		t.Fatalf("GET /applications/7 still rendered the removed Deployment tab: %s", body)
	}
	if strings.Contains(body, `deployments/github-actions`) {
		t.Fatalf("GET /applications/7 still rendered GitHub Actions deployment links: %s", body)
	}
