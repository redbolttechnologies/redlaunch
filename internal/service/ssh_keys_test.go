...
func newSSHKeyTestService(t *testing.T, authorizedKeys string, repository *fakeServerSSHKeyRepository, _ *fakeSSHKeyApplications) *ServerSSHKeyService {
	t.Helper()
	service, err := NewServerSSHKeyServiceWithPath(authorizedKeys, "redlaunch", t.TempDir(), repository, &fakeServerSSHKeyGenerator{})
	if err != nil {
		t.Fatal(err)
	}
	return service
...
func TestServerSSHKeyUsesDedicatedUser(t *testing.T) {
	service, err := NewServerSSHKeyService(t.TempDir(), newFakeServerSSHKeyRepository(), nil, &fakeServerSSHKeyGenerator{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestServerSSHKeyRequiresAbsolutePath(t *testing.T) {
	if _, err := NewServerSSHKeyServiceWithPath("relative/authorized_keys", "redlaunch", t.TempDir(), newFakeServerSSHKeyRepository(), nil); err == nil {
		t.Fatal("relative authorized_keys path was accepted")
	}
	if _, err := NewServerSSHKeyServiceWithPath("/", "redlaunch", t.TempDir(), newFakeServerSSHKeyRepository(), nil); err == nil {
...
func TestServerSSHKeyListHostKeysReadsAndPrefersEd25519(t *testing.T) {
	...
	service, err := NewServerSSHKeyServiceWithHostKeysDir(authorizedKeys, "redlaunch", t.TempDir(), hostKeysDir, newFakeServerSSHKeyRepository(), &fakeServerSSHKeyGenerator{})
	if err != nil {
		t.Fatal(err)
	}
	keys, err := service.ListHostKeys()
...
func TestServerSSHKeyWithHostKeysDirValidation(t *testing.T) {
	authorizedKeys := filepath.Join(t.TempDir(), ".ssh", "authorized_keys")
	if _, err := NewServerSSHKeyServiceWithHostKeysDir(authorizedKeys, "redlaunch", t.TempDir(), "relative/host_keys", newFakeServerSSHKeyRepository(), nil); err == nil {
		t.Fatal("relative host keys dir was accepted")
	}
	disabled, err := NewServerSSHKeyServiceWithHostKeysDir(authorizedKeys, "redlaunch", t.TempDir(), "", newFakeServerSSHKeyRepository(), nil)
	if err != nil {
		t.Fatal(err)
	}
